...
# lib used for creating questions
class Draft(builtins.dict): 
    # use a namespace
    ...
    def __getattr__(self, name):
        try:
            ...
            raise AttributeError(f"'Draft' object has no attribute '{name}'")

    def equal(self, name, expression:str):
        # replace variables in expression with their values defined in the Draft
        # use regex to find all variables in the expression
        replace_regex = re.compile(r'\b(' + '|'.join(map(re.escape, self.keys())) + r')\b')
        def replace_var(match):
            var_name = match.group(0)
            return str(self[var_name])
        evaluated_expression = replace_regex.sub(replace_var, expression)
        self[name] = eval(evaluated_expression)
        return f'{name} = {expression} = {self[name]}'

class Question:
    def __init__(self, question_template:str, default:dict|Draft, answer_template):
        self.question_template = question_template
        self.default = Draft(**default) if not isinstance(default, Draft) else default
        self.answer_template = answer_template

    def gen_answer(self, draft:Draft):
        if callable(self.answer_template):
            return self.answer_template(draft)
        else:
            replace_regex = re.compile(r'\b(' + '|'.join(map(re.escape, draft.keys())) + r')\b')
            def replace_var(match):
                var_name = match.group(0)
                return str(draft[var_name])
            evaluated_expression = replace_regex.sub(replace_var, self.answer_template)
